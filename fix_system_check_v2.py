...
assert "page=page, total_pages=total_pages, total_completed=total_completed)" in lines[1362], f"Line 1363 mismatch: {lines[1362]!r}"
...
<div class="match-card">
  ...
  <div style="font-size:13px;padding:4px 0;display:flex;justify-content:space-between;align-items:center;">
    <span>{{m.top_pred.pick_label}} <span style="color:#888780;">({{"%.1f"|format(m.top_pred.pick_pct)}}%)</span></span>
    ...
    <span class="{{m.top_pred.status}}">{{ {'won':'✅','lost':'❌','pending':'⏳'}[m.top_pred.status] }}</span>
  </div>
  {% if m.other_count > 0 %}
  <a href="/match_detail?league={{m.league}}&fixture_id={{m.fixture_id}}&home={{m.home}}&away={{m.away}}&date={{m.date}}" style="font-size:11px;color:#185FA5;">+{{m.other_count}} други пазара →</a>
  {% endif %}
</div>
{% endmacro %}

<div class="group-title" style="margin-top:20px;">🟢 Чакащи ({{total_pending}})</div>
...
<div style="display:flex;justify-content:center;align-items:center;gap:20px;margin:16px 0;font-size:13px;">
  {% if ppage > 1 %}<a href="?f_league={{filter_league}}&f_market={{filter_market}}&f_status={{filter_status}}&ppage={{ppage-1}}&page={{page}}" style="color:#185FA5;">← Предишна</a>{% endif %}
  <span style="color:#5F5E5A;">Страница {{ppage}} от {{total_pending_pages}}</span>
  {% if ppage < total_pending_pages %}<a href="?f_league={{filter_league}}&f_market={{filter_market}}&f_status={{filter_status}}&ppage={{ppage+1}}&page={{page}}" style="color:#185FA5;">Следваща →</a>{% endif %}
</div>
{% endif %}
...
<div style="display:flex;justify-content:center;align-items:center;gap:20px;margin:24px 0;font-size:13px;">
  {% if page > 1 %}<a href="?f_league={{filter_league}}&f_market={{filter_market}}&f_status={{filter_status}}&page={{page-1}}&ppage={{ppage}}" style="color:#185FA5;">← Предишна</a>{% endif %}
  <span style="color:#5F5E5A;">Страница {{page}} от {{total_pages}}</span>
  {% if page < total_pages %}<a href="?f_league={{filter_league}}&f_market={{filter_market}}&f_status={{filter_status}}&page={{page+1}}&ppage={{ppage}}" style="color:#185FA5;">Следваща →</a>{% endif %}
</div>
...
def system_check():
    ...
    for m in all_matches:
        ...
        m["top_pred"] = max(m["predictions"], key=lambda p: p["pick_pct"])
...
new_lines = lines[:1200] + [new_block] + lines[1363:]

with open(PATH, "w", encoding="utf-8") as f:
    f.writelines(new_lines)

ast.parse(open(PATH, encoding="utf-8").read())
print("OK - system_check v2 приложен")
