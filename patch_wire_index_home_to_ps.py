...
old_block = '''        eligible = [p for p in preds if (p["pick_pct"] or 0) < 95
...
new_block = '''        ranked = ps.rank_logged_rows(preds, league, policy, n=1)
        if not ranked:
            continue
        top_pred = ranked[0]
        top_matches.append({"date": mdate, "home": home, "away": away, "league": league, "top_pred": top_pred})'''
...
print("OK - index_home() вече минава през pick_selection.rank_logged_rows() (Фаза I.1)")
