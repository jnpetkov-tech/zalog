...
assert lines[1169].strip() == 'SYSTEM_CHECK_TEMPLATE = """', f"Line 1170 mismatch: {lines[1169]!r}"
assert "by_league=by_league, matches=matches, cyrillic=to_cyrillic)" in lines[1251], f"Line 1252 mismatch: {lines[1251]!r}"
...
<div class="nav"><a href="/" style="font-weight:500;">🏠 Начало</a><a href="/">Ръчна проверка</a><a href="/daily">Днешни/предстоящи мачове</a><a href="/live">На живо</a><a href="/my_bets">Моите залози</a><a href="/system_check">📊 Проверка</a><a href="/leagues_admin">⚙️ Лиги</a></div>
...
<div class="group-title" style="margin-top:20px;">По мач (най-скорошни първо)</div>
...
<form method="get" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end;margin-bottom:16px;">
  ...
  <div>
    ...
    <select name="f_status" onchange="this.form.submit()">
      ...
      <option value="pending" {% if filter_status=="pending" %}selected{% endif %}>⏳ Чакащи</option>
    </select>
  </div>
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;font-size:13px;color:#5F5E5A;">
  <span>Страница {{page}} от {{total_pages}} ({{total_matches}} мача)</span>
</div>
...
def system_check():
    ...
    all_matches = sorted(match_groups.values(), key=lambda m: m["date"], reverse=True)
    ...
    matches = all_matches[start:start + PAGE_SIZE]

    return render_template_string(SYSTEM_CHECK_TEMPLATE, overall=overall, by_market=by_market,
                                    by_league=by_league, matches=matches, cyrillic=to_cyrillic,
                                    ...
                                    page=page, total_pages=total_pages, total_matches=total_matches)
'''

new_lines = lines[:1169] + [new_block] + lines[1252:]

with open(PATH, "w", encoding="utf-8") as f:
    f.writelines(new_lines)

ast.parse(open(PATH, encoding="utf-8").read())
print("OK - system_check преработен успешно")
