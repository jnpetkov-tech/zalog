...
def team_possession_rating(history_df, ref_date, team):
    home_m = history_df[history_df.home_team == team].dropna(subset=["home_possession"])
    away_m = history_df[history_df.away_team == team].dropna(subset=["away_possession"])
    if home_m.empty and away_m.empty:
        return 50.0

    values = list(home_m["home_possession"]) + list(away_m["away_possession"])
...
def fit_corners_with_possession(history_df, ref_date):
    ...
    poss_ratings = {t: team_possession_rating(history_df, ref_date, t) for t in all_teams}
    league_avg = np.mean(list(poss_ratings.values()))

    home_diff = valid["home_team"].map(poss_ratings).to_numpy() - league_avg
    away_diff = valid["away_team"].map(poss_ratings).to_numpy() - league_avg
    ...
    def nll(params):
        ...
        lam = np.exp(attack[h_idx] - defence[a_idx] + home_adv + beta_poss * home_diff / 10)
        mu = np.exp(attack[a_idx] - defence[h_idx] + beta_poss * away_diff / 10)
        ll = poisson.logpmf(hv, lam) + poisson.logpmf(av, mu)
        return -np.sum(ll * weights)

    x0 = np.zeros(2 * n + 2)
    result = minimize(nll, x0, method="L-BFGS-B")
    return result.x[:n], result.x[n:2*n], result.x[-2], result.x[-1], poss_ratings, league_avg


def predict_total(attack, defence, home_adv, beta, ratings, league_avg, home, away, max_val=20):
...
for i, row in test_df.iterrows():
    if i % RETRAIN_EVERY == 0:
        ...
        model = fit_corners_with_possession(history, row["date"])
...
print(f"Среден fitted beta_possession коефициент: {np.mean(beta_values):.4f}")
print(f"Corners O/U {THRESHOLD} С possession rating: {correct/total*100:.1f}%")
print(f"(n={total})")
