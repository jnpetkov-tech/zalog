...
# 22.09.2026 (ZADACHA_PAZARI.md, ЧАСТ А): премерено върху ~780 уредени реда на пазар - виж
# validation/blend_weights_v2_20260922.md. draw/over25/under25 -> 1.0 (бият старото тегло на
# ДВЕТЕ хронологични половини); home_win/away_win без промяна (away 0.8 не бие на 1-ва половина).
# 23.09.2026 (ZADACHA_MODEL1.md, ЧАСТ Б, решение на Дака): смесването е ИЗКЛЮЧЕНО - всички 0.0,
# публикуваме чистия модел. Пазарът се записва невидимо (predictions_log.market_pct) като мерило
# (validation/model_vs_market.py), не като съставка. Машинарията остава - включва се пак със
# смяна на числата тук. Предишни стойности: home 1.0, draw 1.0, away 0.7, over25/under25 1.0.
# Ефектът преди/след: validation/blend_off_impact_20260923.txt.
BLEND_WEIGHTS = {
    "home_win": 0.0,
    "draw": 0.0,
    "away_win": 0.0,
...
# доказва (един мач не е доказателство). Разговор с Дака, 24.08.2026 (т.4):
# вместо да сменим прага по усещане, измерихме го - validation/
# ev_threshold_backtest_20260824.csv, кофа "40%+" срещу реални уредени
# резултати: n=210, ROI -41%, 95% bootstrap CI [-0.636,-0.162] (изключва
# нулата - значимо, не шум). Прагът остава 40% - вече със СЪЩОТО число, но
# подкрепено от истинско покритие (n=210), не от един мач. Долните 0-40%
# кофи (0-2/2-5/5-10/10-20%) НЕ излизат статистически различими от шум с
# наличните данни - затова MIN_VALUE_BET_PROB/долната граница на EV НЕ се
# пипат тук, няма основание да се предпочете 0% пред 3% или обратно.
MAX_TRUSTWORTHY_EV = 0.40  # 40% - над това не вярваме на модела си (виж validation/ev_threshold_backtest_20260824.csv)
