...
def write_md(tab, lgt, best, control, early, has_players, base):
    ...
    L = [f"# Сила на играч / състав като ковариата - {TAG}", "",
         ...
         "## Резултат", "",
         f"Настройки, по-добри от сегашния модел на ранната половина: {int((tab['d_ранна'] < 0).sum())} от {len(tab) - 1}"
         + (" - т.е. правилото за избор всъщност оставя сегашния модел; долу е най-малко лошата, за пълнота."
            if not (tab['d_ранна'] < 0).any() else "."), "",
