...
def fit_goals_model(history_df, ref_date, team_idx, n, cov_home_col=None, cov_away_col=None, xi=None,
    ...
    obs_cols (ZADACHA_KALIBRACIQ.md, ЧАСТ Б, 23.09.2026): (домакин, гост)
    колони, от които моделът учи вместо home_goals/away_goals - напр.
    ("home_xg", "away_xg"). Наблюденията могат да са дробни, затова
    ...
    if obs_cols is not None:
        if use_dc:
            raise ValueError("obs_cols изисква use_dc=False")
        valid = history_df.dropna(subset=list(obs_cols))
        hg = valid[obs_cols[0]].to_numpy(dtype=float)
        ag = valid[obs_cols[1]].to_numpy(dtype=float)
    else:
    ...
    def nll(params):
        ...
        if obs_cols is None:
            ...
            ll = (xlogy(hg, lam) - lam - gammaln(hg + 1)) + (xlogy(ag, mu) - mu - gammaln(ag + 1))
        if use_dc:
