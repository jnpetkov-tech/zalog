...
for league in LEAGUES:
    ...
    test_df = df[
        ...
        df["h2h_avg_goals"].notna() &
        (df["h2h_count"] >= MIN_MEETINGS)
    ].reset_index(drop=True)

    if len(test_df) < 15:
        print(f"Недостатъчно мачове със {MIN_MEETINGS}+ H2H срещи ({len(test_df)}), пропускам.")
        continue
    ...
    print(f"Мачове с {MIN_MEETINGS}+ H2H срещи: {total}")
